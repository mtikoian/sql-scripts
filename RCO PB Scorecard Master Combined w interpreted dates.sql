...
-->>>>>>>>  Calculation for Open Encounters < 8 days  <<<<<<<<<<<
		sum(Case
			When datediff(day,"PAT_ENC"."CONTACT_DATE",@end_date)<7 Then 1
		End) as "<7 Days",

-->>>>>>>>  Calculation for Open Encounters 7 - 30 days  <<<<<<<<<<<
		sum(Case
			When datediff(day,"PAT_ENC"."CONTACT_DATE",@end_date)>6 and datediff(day,"PAT_ENC"."CONTACT_DATE",@end_date)<31 Then 1
		End) as "7 - 30 Days",
...
-->>>>>>>>  Calculation for Lag Days  <<<<<<<<<<<
 ...
 WHERE  "ARPB_TX_VOID"."OLD_ETR_ID" IS  NULL  AND "ARPB_TRANSACTIONS"."AMOUNT"<>0 AND 
		("ARPB_TRANSACTIONS"."POST_DATE">= @start_date AND 
		 "ARPB_TRANSACTIONS"."POST_DATE"<= @end_date) AND 
...
/*(select monthname_year as 'report date'
,count(calendar_dt) as 'days'
from V_CUBE_D_BILLING_DATES
where (calendar_dt >= @start_date
and calendar_dt <= @end_date)
group by monthname_year, calendar_dt
)as rpt_date
,
*/