...
/****** Object:  View [Rpt].[V_CHP_CUBE_F_PB_TRANSACTION_DETAIL]    Script Date: 1/30/2016 8:35:06 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [Rpt].[V_CHP_CUBE_F_PB_TRANSACTION_DETAIL]
(
 [TDL_ID]
,[DETAIL_TYPE]
,[TYPE]
...
,[PATIENT_PAYMENTS]
,[INSURANCE_PAYMENTS]
,[DEBIT_ADJUSTMENTS_OTHER]

)
AS
/*Copyright (C) 2014 Epic Systems Corporation
********************************************************************************
...
--,Case when tdl.detail_type in (3,12,22,23) then coalesce(tdl.proc_id,-1)
--else -5 end --78
,case when tdl.detail_type in (20,21,22,23) then coalesce(arpb_match.loc_id,-2) else coalesce(tdl.loc_id,-2) end       --79
,cast((case when tdl.detail_type in (21,23) and eap_match.gl_num_debit = 'Admin' then tdl.amount
	  ...
	  else 0 end) as numeric(20,2)) as CREDIT_ADJ_ADMIN          --80
,cast((case when tdl.detail_type in (21,23) and eap_match.gl_num_debit = 'CHARITY' then tdl.amount
	  ...
	  else 0 end) as numeric(20,2)) as  CREDIT_ADJ_CHARITY        --81
,cast((case when tdl.detail_type in (21,23) and eap_match.gl_num_debit = 'CONTRA' then tdl.amount
	  ...
	  else 0 end) as numeric(20,2))  as CREDIT_ADJ_CONTRA    --82
,cast((case when tdl.detail_type in (21,23) and eap_match.gl_num_debit = 'BAD' then tdl.amount
	  ...
	  else 0 end) as numeric(20,2))  as CREDIT_ADJ_BAD_DEBT    --83
,cast((case when tdl.detail_type in (3,12) and tdl.credit_gl_num = 'REFUND' then tdl.amount
	  else 0 end) as numeric(20,2))  as DEBIT_ADJ_REFUND            --84
,cast((case when tdl.amount is null then 0
      else tdl.amount end) as numeric(20,2))  as NET_CHANGE_AR         --85
,cast((case when tdl.detail_type in (1,10,40,41,42,43,44,45) and month(tdl.post_date) = month(tdl.orig_service_date) then tdl.amount
      else 0 end) as numeric(20,2))  as CHARGES_CURRENT   --86
,cast(((CASE WHEN tdl.DETAIL_TYPE IN (1,10) THEN tdl.ACTIVE_AR_AMOUNT ELSE 0 END) - (case when tdl.detail_type in (1,10,40,41,42,43,44,45) and month(tdl.post_date) = month(tdl.orig_service_date) then tdl.amount
      else 0 end)) as numeric(20,2))  as CHARGES_LATE    --87
,cast((case when tdl.detail_type in (2,5,11,20,22,32,33) then tdl.patient_amount else 0 end) as numeric(20,2))  as PATIENT_PAYMENTS     --88
,cast((case when tdl.detail_type in (2,5,11,20,22,32,33) then tdl.insurance_amount else 0 end) as numeric(20,2))  as INSURANCE_PAYMENTS --89
,cast((case when tdl.detail_type in (3,12) and eap.gl_num_debit is null or eap.gl_num_credit <> 'REFUND' then tdl.amount else 0 end) as numeric(20,2))  as DEBIT_ADJUSTMENTS_OTHER

FROM [CLARITY]..[CLARITY_TDL_TRAN] tdl
left outer join [CLARITY]..ARPB_TRANSACTIONS tx on tdl.TX_ID = tx.TX_ID
left outer join [CLARITY]..CLARITY_UCL ucl on tx.CHG_ROUTER_SRC_ID = ucl.UCL_ID
left outer join [CLARITY]..ARPB_TX_MODERATE moderate on tdl.TX_ID = moderate.TX_ID
left outer join [CLARITY]..ARPB_AGING_HISTORY hx on hx.tx_id=tdl.match_trx_id AND tdl.post_date>=hx.SNAP_START_DATE and tdl.POST_DATE<=HX.SNAP_END_DATE
left outer join [CLARITY]..ARPB_TX_VOID void on tdl.tx_ID=void.TX_ID
left outer join first_entry_date fed on fed.TAR_ID=moderate.ORIGINATING_TAR_ID
left outer join [CLARITY]..ZC_PAYMENT_SOURCE paysrc on tdl.payment_source_c=paysrc.payment_source_c
left outer join [CLARITY]..ARPB_TRANSACTIONS arpb_match on tdl.match_trx_id=arpb_match.tx_id
left outer join [CLARITY]..ZC_TRAN_TYPE tran_type on tdl.tran_type = tran_type.tran_type
left outer join [CLARITY]..ARPB_TRANSACTIONS2 tx2 on tdl.TX_ID = tx2.TX_ID
...
left outer join [CLARITY]..CLARITY_EAP eap on tdl.proc_id = eap.proc_id
left outer join [CLARITY]..CLARITY_EAP eap_match on tdl.match_proc_id = eap_match.proc_id
where tdl.DETAIL_TYPE<39 and tdl.tran_type<=3
AND tdl.post_date>='2015-01-01'
