...
case when loc.loc_id in (11106,11124,11149,19147)  then 'SPRINGFIELD'
	 when loc.loc_id in (11101,11102,11103,11104,11105,11115,11116,11122,11139,11140,11141,11142,11143,11144,11146,11151,11132,11138,19151,19154,19148,19155,19156,19149,19150) then 'CINCINNATI'
...
from region
inner join clarity_tdl_age age on age.DEPT_ID = region.DEPARTMENT_ID
...
inner join region on region.DEPARTMENT_ID = enc.department_id
...
-->>>>>>>>  Calculation for Lag Days  <<<<<<<<<<<
 ...
 FROM region
 inner join "Clarity"."dbo"."ARPB_TRANSACTIONS" ARPB_TRANSACTIONS on arpb_transactions.DEPARTMENT_ID = region.DEPARTMENT_ID
...
((((("Clarity"."dbo"."PAT_ENC" "PAT_ENC" INNER JOIN "Clarity"."dbo"."PAT_ENC_2" "PAT_ENC_2" ON ("PAT_ENC"."PAT_ENC_CSN_ID"="PAT_ENC_2"."PAT_ENC_CSN_ID") AND ("PAT_ENC"."CONTACT_DATE"="PAT_ENC_2"."CONTACT_DATE")) 
																				   ...
																				   left join clarity_loc loc on loc.loc_id = dep.rev_loc_id
																				   inner join region on region.DEPARTMENT_ID = dep.department_id
 WHERE 
