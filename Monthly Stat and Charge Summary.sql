...
select 
 serv_area_id as 'Service Area ID'
,case when serv_area_id is null then 'NO SERVICE AREA' else serv_area_id + ' - ' + serv_area_name end as 'Service Area'
,case when loc_name is null then 'NO LOCATION' 
     when loc_gl_prefix is null then 'NO LOCATION GL'
	 else loc_gl_prefix + ' - ' + loc_name end as 'Location'
,case when department_name is null then 'NO DEPARTMENT' 
     when dep_gl_prefix is null then 'NO DEPARTMENT GL'
	 else dep_gl_prefix + ' - ' + department_name end as 'Department'
,billing_provider_id + ' - ' + billing_prov_name as 'Service Provider'
,cpt_code + ' - ' + proc_name as 'CPT Code'
,sum(rvu_work) as 'wRVU'
,sum(adj_wrvu) as 'Adjusted wRVU'
,sum(total_rvu) as 'Total wRVU'
,sum(amount) as 'Charges'
,sum(procedure_quantity) as 'Units'
...
order by 
 serv_area_id + ' - ' + serv_area_name
,loc_gl_prefix + ' - ' + loc_name
,dep_gl_prefix + ' - ' + department_name
,billing_provider_id + ' - ' + billing_prov_name
,cpt_code + ' - ' + proc_name
