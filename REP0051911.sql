...
SELECT '10' AS HSPCODE, CAST(HSP_TRANSACTIONS.TX_ID AS varchar(30)) AS PaymentID, 
                  CLARITY_EAP.PROC_CODE AS TransactionCode, HSP_TRANSACTIONS.TX_AMOUNT AS TransactionAmount, REPLACE(CONVERT(varchar(10), 
                  HSP_TRANSACTIONS.TX_POST_DATE, 101), '/', '') AS TransactionPostDate
FROM     HSP_TRANSACTIONS INNER JOIN
                  CLARITY_EAP ON HSP_TRANSACTIONS.PROC_ID = CLARITY_EAP.PROC_ID INNER JOIN
     clarity_report.hsp_account_exclude_LPPI as             HSP_ACCOUNT ON HSP_TRANSACTIONS.HSP_ACCOUNT_ID = HSP_ACCOUNT.HSP_ACCOUNT_ID INNER JOIN
...
WHERE  (HSP_TRANSACTIONS.TX_TYPE_HA_C = 2) 

/*AND (cast(HSP_account.acct_close_date as date) >= @startdate) AND 
                  (cast(HSP_account.acct_close_date as date) <= @enddate)*/